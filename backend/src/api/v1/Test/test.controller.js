...
// update test references - dont change it it is a fine fix if you update this the binding will break 26/06/2025 by Shreyash
export async function updateTestReferences(modelName, docId, newName) {
  try {
    const pathMap = {
      Class: 'classes',
      Subject: 'subjects',
...
const createTest = async (req, res) => {
  try {
    ...
    if (payload.type === 'Descriptive') {
      const homeworkPayload = {
        subjectId: [payload.subjects.map((subject) => subject._id)],
        classId: [payload.classes.map((classs) => classs._id)],
        bookId: [payload.book.map((book) => book._id)],
        lessonId: [payload.lesson.map((lesson) => lesson._id)],
        topicId: payload?.topic?.length
          ? payload.topic.map((topic) => topic?._id)
          : undefined,

        question: payload.description,
        solution: payload.descriptionSol,
        hint: '',
        videoTutorialLink: '',
        createdBy: req.user._id,
      };
      await homeworkModel.create(homeworkPayload);
    }
...
const getAllTest = async (req, res) => {
  try {
    ...
    if (userRole !== 'admin') {
      ...
      // return only those which profileDate.subjectId , bookId, classId
    }
    if (search) {
    ...
    query.isLast = true;
    console.log(query, 'quuu');
    let [list, totalCount] = await Promise.all([
      Test.find(query)
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
...
const publishTest = async (req, res) => {
  try {
    ...
    for (const test of tests) {
      ...
      let users = await userprofileModel
        .find({
          subjectId: { $in: subjectIds },
          classId: { $in: classIds },
        })
        .populate('userId');
      let userData = await userprofileModel.findOne({ userId: req.user._id });
      let batch = await batchModel.findOne({
        classTeacherId: userData.userId,
      });
      if (!batch) {
        batch = await batchModel.findById(userData.joinedBatch);
        // find joind techers students
        // users =
      }
      // userData?.joinedBatch
      ...
      const notifications = users
        ...
        .filter(
          (user) =>
            batch?._id && user?.joinedBatch?.toString() === batchId.toString()
        )
        .map((user) => ({
          batchId: batchId,
      ...
      // find all techers for that batch
...
const getPublishedTest = async (req, res) => {
  try {
    ...
    const { subjectId, classId, batchId } = req.query;
      ...
      testData.map(async (test) => {
        try {
          ...
          const normalizeTimeString = (timeStr) =>
            timeStr.replace(/[\u202F\u00A0]/g, ' ').trim(); // Replace non-breaking spaces

          const cleanedTime = normalizeTimeString(test.publishedTime);
          console.log('Cleaned time:', cleanedTime);
          const parsedTime = moment(cleanedTime, 'h:mm A', true);

          if (!parsedTime.isValid()) {
            console.log(
              `Invalid publishedTime for test ${test._id.toString()}: ${test.publishedTime}`
            );
            return null;
          }
          ...
          if (!publishedDate.isValid()) {
            console.log(
              `Invalid publishedDate for test ${test._id.toString()} ${test.publishedDate}`
            );
            return null;
          }
...
const previousDayTests = async (req, res) => {
  try {
    ...
    const { classId, subjectId, batchId } = req.query;
...
const studentTestMarks = async (req, res) => {
  try {
    ...
    const batchIds = [
      teacherBatch?._id?.toString(),
      userProfile?.joinedBatch?.toString(),
    ];
    ...
    joinedStudentIds.forEach(async (studentId) => {
      let user = await User.findById(studentId);

      if (user?.userRole !== 'student') {
        joinedStudentIds.splice(joinedStudentIds.indexOf(studentId), 1);
      }
    });
    ...
    console.log('joinedStudentIds', submittedData.length);
      ...
      submission.test.forEach((testItem, index) => {
        ...
        if (question.type === 'True/False') {
          ...
          for (let i = 0; i < 4; i++) {
            if (optionBooleans[i]) {
              correctOptions.push({
                text: normalize(optionTexts[i]),
                mark: optionMarks[i],
              });
            }
          }

          const normalizedUserAnswers = userAnswers.map((a) =>
            normalize(String(a))
          );
          let questionObtainedMarks = 0;

          const allCorrectAnswers = correctOptions.map((opt) => opt.text);
          const hasWrongAnswer = normalizedUserAnswers.some(
            (ans) => !allCorrectAnswers.includes(ans)
          );

          if (!hasWrongAnswer) {
            normalizedUserAnswers.forEach((ans) => {
              const match = correctOptions.find((opt) => opt.text === ans);
              if (match) {
                questionObtainedMarks += match.mark;
              }
            });
          } else {
  ...
  try {
    ...
    const groupedSubmissions = submittedData.reduce((acc, submission) => {
      ...
      submission.test.forEach((testItem) => {
        ...
        const normalize = (val) => (val || '').toString().trim().toLowerCase();
        if (question.type === 'True/False') {
          const correctAnswer = normalize(String(question.optionTrue));
          const userAnswer = normalize(String(userAnswers?.[0] || ''));
          ...
          const assertionTexts = [
            question.optionAssertionText1,
            question.optionAssertionText2,
            question.optionAssertionText3,
            question.optionAssertionText4,
          ];
          const assertionBooleans = [
            question.optionAssertion1,
            question.optionAssertion2,
            question.optionAssertion3,
            question.optionAssertion4,
          ];
          const assertionMarks = [
            parseFloat(question.markAssertion1) || 0,
            parseFloat(question.markAssertion2) || 0,
          ...
          for (let i = 0; i < 4; i++) {
            if (assertionBooleans[i]) {
              correctAssertions.push({
                text: normalize(assertionTexts[i]),
                mark: assertionMarks[i],
              });
            }
          }
          ...
          normalizedUserAnswers.forEach((ans) => {
            const match = correctAssertions.find((opt) => opt.text === ans);
            if (match) questionObtainedMarks += match.mark;
          });
...
const getTestsByLessonAndTopic = async (req, res) => {
  try {
    ...
    // });
    const tests = await Test.find({
      'lesson._id': new mongoose.Types.ObjectId(lessonId),
      isLast: true,
      testType: 'online',
      displayTrue: true,
    }).populate({
      path: 'topic._id',
      match: { isTestRequired: true },
    });
      ...
      .map((key) => {
        ...
        const data = Object.entries(userMarks)
          ...
          .map(([userId, data]) => ({
            ...
            totalObtained: data.totalObtained.toString() || 0, // Fallback for missing totalObtained
