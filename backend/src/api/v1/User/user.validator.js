...
export const createUserValidation = yup.object().shape({
  name: yup.object().shape({
    english: yup.string().trim().required('Name in English is required').min(3, 'Name must have more than 2 characters'),
    hindi: yup.string().trim().default(''),
    hinglish: yup.string().trim().default(''),
  }),
  email: yup
    .string()
    .trim()
...
export const mobileLoginUserValidation = yup.object().shape({
  mpin: yup.string().required('MPIN is required').length(4, 'Must be exactly 4 characters'),
