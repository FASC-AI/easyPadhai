...
import SingleSelector from "./singledropdown";
...
const SingleSelectorSelector = ({
  ...
  isMulti = true, // New prop to toggle between multi and single select
}) => {
  ...
  const toggleSelection = (option) => {
    ...
    if (isMulti) {
      ...
      // Single select: replace current selection with new option
      updated = selected.some((s) => s.value === option.value) ? [] : [option];
  ...
  const selectAll = () => {
    if (!isMulti) return; // Disable select all in single-select mode
      ...
      >
        ...
        <div className="max-h-[200px] overflow-y-auto text-[12px] text-gray-600 custom-scrollbar">
          {isLoading ? (
            ...
            <>
              {isMulti && Object.values(filteredOptions).flat().length > 0 && (
                <div
...
SingleSelectorSelector.displayName = "MultipleSelector";
export default SingleSelectorSelector;
