  ...
  return (
    <div className="list-tab">
      <div className="tabs ">
        {tabs.map((item, index) => (
          <div
            className={` tab ${isActiveTab === item.tabName ? 'tab-active' : ''
              }`}
            key={index}
            onClick={() => setIsActiveTab(item.tabName)}
          >
            <div
              className={` ${isActiveTab === item.tabName
...
export const ResizedTabs = ({ tabs, isActiveTabs, hasActiveTabHandler }) => {
  ...
  const handleResize = () => {
    if (window.innerWidth < 500) {
      ...
      setAllTabs({
        tabList: tabs.slice(0, 5),
        extraTabList: tabs.slice(5),
      });
    } else if (window.innerWidth < 1280) {
      setAllTabs({
        tabList: tabs.slice(0, 7),
        extraTabList: tabs.slice(7),
      });
    }
    else if (window.innerWidth < 1366) {
      setAllTabs({
        tabList: tabs.slice(0, 10),
        extraTabList: tabs.slice(10),
      });
    }
    else if (window.innerWidth < 1552) {
      setAllTabs({
        tabList: tabs.slice(0, 11),
        extraTabList: tabs.slice(11),
      });
    }
    else if (window.innerWidth < 1680) {
      setAllTabs({
        tabList: tabs.slice(0, 12),
        extraTabList: tabs.slice(12),
      });
    }
    else if (window.innerWidth < 1780) {
      setAllTabs({
        tabList: tabs.slice(0, 13),
        extraTabList: tabs.slice(13),
      });
    }
    else {
      setAllTabs({
        tabList: tabs.slice(0, 20),
        extraTabList: tabs.slice(20),
      });
    }
  };
  useEffect(() => {
  ...
  return (
    <div className="tabs">
      {allTabs.tabList.map((item, index) => (
        <div
          className={` tab ${isActiveTabs === item.tabName
      ...
      {allTabs.extraTabList.length > 0 && ( // Only render "More" if there are extra tabs
        <div
          className={`tab relative ${allTabs.extraTabList.some((tab) => tab.tabName === isActiveTabs)
              ? 'border-b-2 border-blue-primary-200'
              : ''
            }`}
        >
          <div
            className={` ${allTabs.extraTabList.some((tab) => tab.tabName === isActiveTabs)
                ? 'text-sm text-blue-primary-200'
                : 'text-sm text-gray-tertiary hover:text-gray-secondary'
          ...
          {showDropdown && (
            <>
              ...
              />
              <section className="absolute top-10 right-0 bg-white rounded-md shadow h-[200px] z-[99] overflow-scroll p-1">
