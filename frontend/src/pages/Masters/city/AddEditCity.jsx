...
const AddEditCity = () => {
  ...
  useEffect(() => {
    if (id && !hasFetchedCity.current) {
      setLoading(true);
      getApi(APIS.CITY, id)
        .then((res) => {
          const editData = res?.data?.city;
          const country = editData?.country;
          const state = editData?.state;
          const district = editData?.district;
          setData({
            ...initialValues,
            lgd: editData.lgd,
          ...
          const filteredDistrict = allDistrict.filter(
            (district) => district.stateId === state?._id
          );
  ...
  const handleStateChange = (option, setFieldValue) => {
    ...
    const filteredDistrict = option
      ? allDistrict.filter((district) => district.stateId === option.value)
      : [];
    setDistrictOptions(filteredDistrict);

    setSelectedDistrict(null);
    setFieldValue("district", { _id: "", name: "" });
  };
    ...
    if (id) {
      ...
      patchApi(APIS.CITY, id, payload)
        .then(() => {
          ...
          if (!values.district._id && values.state._id) {
            const filteredDistrict = allDistrict.filter(
              (district) => district.stateId === values.state._id
            );
            setDistrictOptions(filteredDistrict);
          }
          navigate(-1);
              ...
              <>
                <Header>
                  <div>
                    <BreadCrumbs
                      backNavi={() => navigate(-1)}
                      breadCrumbs={[{ name: "Cities", path: ROUTES.MASTERS }]}
                      boldItem={"Add City"}
                    />
                    <Heading>{id ? "Edit" : "Add"} City</Heading>
                  </div>
                  <ButtonContainer>
                    <Button
