...
const CountryListView = () => {
  ...
  const handleConfirm = async () => {
    try {
      const response = await apiService.delete(`${APIS.COUNTRY}/${CountryId}`);
  ...
  const navigateToEdit = (rowData) => {
    navigate(`${ROUTES.UPDATE_COUNTRY}/${rowData?._id}`, {
...
// },
  ...
  const HEAD_BUTTONS = [
    ...
    //   label: 'Total Stock',
    //   children: (
    //     <div CountryName={cn(buttonVariants(), '')}>
    ...
    {
      label: 'Add Country',
      children: <Button onClick={() => handleAddUser()} CountryName="bg-[#1A6FAB] text-white hover:bg-[#1A6FAB] hover:text-white">+ Add Country</Button>,
    },
    {
      label: 'More',
      children: (
  ...
  const fetchData = async ({ page, pageSize, search, sortBy, order, tab }) => {
    ...
    const response = await apiService.get(`${APIS.COUNTRY}`, {
      params,
    });
