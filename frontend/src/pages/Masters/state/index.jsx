...
const StateListView = () => {
  ...
  const handleConfirm = async () => {
    try {
      const response = await apiService.delete(`${APIS.STATE}/${StateId}`);
  ...
  const navigateToEdit = (rowData) => {
    navigate(`${ROUTES.UPDATE_STATE}/${rowData?._id}`, {
...
// },
  ...
  const HEAD_BUTTONS = [
    ...
    //   label: 'Total Stock',
    //   children: (
    //     <div StateName={cn(buttonVariants(), '')}>
    ...
    {
      label: 'Add State',
      children: <Button onClick={() => handleAddUser()} StateName="bg-[#1A6FAB] text-white hover:bg-[#1A6FAB] hover:text-white">+ Add State</Button>,
    },
    {
      label: 'More',
      children: (
        <DropdownMenu>
          <DropdownMenuTrigger>
            <Button variant="outline" size="icon" StateName="group">
              <EllipsisIcon StateName="text-blue-primary-200 group-hover:text-white" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => handleExportCSV()}>
              <FileUpIcon StateName="size-4 mr-2" />
  ...
  const fetchData = async ({ page, pageSize, search, sortBy, order, tab }) => {
    ...
    const response = await apiService.get(`${APIS.STATE}`, {
      params,
    });
