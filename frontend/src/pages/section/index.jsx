...
const SectionsListView = () => {
  ...
  useEffect(() => {
    const breadcrumbData = [{ label: "Section" }];
    updateBreadcrumb(breadcrumbData);
  }, [updateBreadcrumb]);

  const navigateToEdit = (rowData) => {
    navigate(`${ROUTES.UPDATE_SECTION}/${rowData?._id}`, {
  ...
  const handleStatusToggle = async (row, status) => {
    ...
    try {
      const response = await apiService.patch(
        `${APIS.SECTION}/${row._id}`,
        payload
      );
      if (response?.status === true && response?.code === 200) {
        toast.success(`Section has been marked as ${status} successfully`);
  ...
  const handleDelete = async () => {
    try {
      const response = await apiService.delete(`${APIS.SECTION}/${sectionId}`);
  ...
  const handleDragEnd = async ({ oldIndex, newIndex, newData }) => {
    try {
      const orderPayload = newData.map((section, index) => ({
        id: section._id,
        order: index,
      }));
      const response = await apiService.post(`${APIS.SECTION_REORDER}`, {
        order: orderPayload,
      });
      if (response?.code === 200 && response?.status === true) {
        ...
        toast.error(response?.message || "Failed to update section order");
      }
    } catch (error) {
      toast.error(
  ...
  const fetchData = async ({ page, pageSize, search, sortBy, order, tab }) => {
    try {
      ...
      const response = await apiService.get(APIS.SECTION_LIST, { params });
      const data = response.data.section.map((row) => ({
        ...row,
        id: row._id,
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={(e) => {
                  e.stopPropagation();
                  handleDeleteSection(params.row._id);
                }}
                disabled={isInactive}
              >
      ...
      <QueryClientProvider client={queryClient}>
        ...
        {confirmationPopup.isOpen && (
          <PopupAlert
            ...
            message={`Are you sure you want to mark this Section as ${confirmationPopup.action}?`}
        ...
        <PopupAlert
          ...
          title="Delete"
          message="Are you absolutely sure you want to delete? This action cannot be undone."
