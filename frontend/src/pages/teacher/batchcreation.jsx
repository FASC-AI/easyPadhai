...
const TeacherCreationListView = () => {
  ...
  const fetchData = async ({ page, pageSize, search, sortBy, order, tab }) => {
    ...
    try {
      ...
      if (response?.data?.batchCodes?.length) {
        const mappedData = response.data.batchCodes.map((item, index) => {
        
          return {
            id: item._id || index,  // Ensure a unique fallback
            batchCreatedcode: item.code || '--',
            batchCreatedclass: item.classId || {},
            batchFollowsubject: item.sectionId ? [item.sectionId] : [],
            space:'',
          };
        });
  ...
  const handleDeleteUser = (id) => {
   ...
   const breadcrumbItems = [
    ...
    { name: "Teacher", path: ROUTES.TEACHER_LIST },
...
export default TeacherCreationListView;
